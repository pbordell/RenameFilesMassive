...
import com.pbs.ui.AboutDialog;
import com.pbs.ui.SliderBar;
import com.pbs.ui.WarningDialog;
import com.pbs.util.DateTimeUtil;
import com.pbs.util.FileUtils;
...
public class MediaPlayerController implements Initializable {
	...
	private void playVideo(String MEDIA_URL) {
		try {
			...
			MEDIA_URL_FOR_MEDIA = "file:/" + (MEDIA_URL_FOR_MEDIA).replace("\\", "/").replace("+", "%20");
	...
	private void bindMediaPlayerControls(final MediaPlayer mediaPlayer) {
		...
		timeSlider.sliderValueProperty().addListener((ov) -> {
			if (timeSlider.isTheValueChanging()) {
				if (null != mediaPlayer)
					...
					mediaPlayer.seek(duration.multiply(timeSlider.sliderValueProperty().getValue() / 100.0));
			}
		});
		...
		volumeSlider.sliderValueProperty().addListener((ov) -> {
			if (null != mediaPlayer) {
				...
				mediaPlayer.setVolume(volumeSlider.sliderValueProperty().getValue() / 100.0);
			}
		});
		...
		if (playTime != null && timeSlider != null && volumeSlider != null) {
			Platform.runLater(() -> {
				...
				if (!timeSlider.isDisabled() && duration.greaterThan(Duration.ZERO)
					...
					timeSlider.sliderValueProperty()
			...
			scene.setOnDragDropped((dragEvent) -> {
				...
				if (db.hasFiles()) {
					for (Path filePath : FileUtils.convertListFiletoListPath(db.getFiles())) {
						try {
							if (PropertiesUtils.readFormats().contains("*" + filePath.toAbsolutePath().toString()
									.substring(filePath.toAbsolutePath().toString().length() - 4))) {
								if (null != mediaView.getMediaPlayer())
